...
| `pnpm analyze:ci` | Fallow audit in JSON mode (local only; CI uses the fallow GitHub Action) |
| `pnpm mcp` | Start the MCP server (`src/mcp/server.ts`) |
...
- **Unit / integration (Vitest + React Testing Library)**: markdown pipeline (sanitization, smart quotes, code highlighting, YouTube parsing), `generateId`, `generateTitle`, `localSync` (BroadcastChannel fallback), `useDocument` hook, `useDocuments` hook, `useWorkspace` hook, `SplitPane`, `Split`, `DocumentMenu`, `Textarea`, `Button`, `Wordmark`, `WorkspaceDrawer`, `WorkspaceCreate`, and the Express server (CSP headers, helmet, SPA fallback) via `supertest`.
- **End-to-end (Playwright, Chromium)**: full document flow, multi-tab concurrent edits, localStorage persistence, XSS sanitization, document menu navigation, mode switching (split / read / write), workspace flows.
...
Four jobs run in parallel on every push and PR, all blocking — a PR cannot merge unless all four pass:

- **Biome** — lint and format check (`pnpm check`). Fails on any lint error or unformatted file.
- **Vitest** — unit and integration tests with v8 coverage. Coverage report uploaded as an artifact (14-day retention).
- **Playwright** — end-to-end tests on Chromium. Playwright report and traces uploaded as an artifact (14-day retention).
- **Fallow** — dead code, duplication, and complexity audit against baseline files. Fails on any regression.
...
| Icons | Lucide React components (`lucide-react`) rendered as inline SVGs |
...
│   ├── WorkspaceDrawer.tsx      # Workspace slide-in drawer
│   ├── WorkspaceCreate.tsx      # UI for creating a new workspace
│   ├── FolderTree.tsx           # Folder/document tree within a workspace
│   ├── ContextMenu.tsx          # Right-click context menu
│   ├── Split.tsx                # Generic split-pane layout primitive
│   ├── Button.tsx               # Shared button component
│   └── Wordmark.tsx             # App wordmark / logo
├── design/
...
│   └── useWorkspace.ts          # Workspace state and management
├── mcp/
